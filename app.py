...
def index():
    ...
    for project in projects:
        ...
        project['date'] = time.strftime(
            '%Y-%m-%d %H:%M', time.localtime((recentDate / 1000))
        )
        if unreviewed:
    ...
    if uwsgi.cache_exists("delivered", "igoqc"):
        ...
        delReq = s.get(
            LIMS_API_ROOT + "/LimsRest/getRecentDeliveries?time=2&units=d",
            auth=(USER, PASSW),
            verify=False,
        )
...
def build_grid_from_samples(samples, pType):
    ...
    for sample in samples:
        if (
            sample['qc']['qcStatus'] != "Failed"
            and sample['qc']['qcStatus'] != "Failed-Reprocess"
        ):
    ...
    for sample in samples:
        ...
        cov_target = (
            "" if sample['coverageTarget'] == 0 else sample['coverageTarget']
        )  # hack; coverage target is set to 0 in LIMS by default at pull, will display as empty string on site
        grid.set_value("Coverage Target", row, format_int(cov_target))
        grid.set_value(
            "Requested Reads (Millions)",
            row,
            format_int(sample['requestedNumberOfReads']),
        )
        ...
        grid.set_value(
            "Pct. Duplic.", row, format_fp(qc['percentDuplication'] * 100)
        )  #
        if pType["startable"]:
        ...
        if pType["qcControlled"]:
            grid.set_value(
                ...
                row,
                "{:,.2f}".format(qc["qcControl"]) + " " + str(qc["qcUnits"]),
            )
        if pType["quanted"]:
            grid.set_value(
                "Quant-it",
                row,
                "{:,.2f}".format(qc["quantIt"]) + " " + qc["quantUnits"],
            )
        ...
        if pType['table'] == 'rna':
            grid.set_value(
                "Pct. Ribos.", row, format_fp(qc['percentRibosomalBases'] * 100)
            )
            grid.set_value(
                "Pct. Coding", row, format_fp(qc['percentCodingBases'] * 100)
            )
            grid.set_value("Pct. Utr", row, format_fp(qc['percentUtrBases'] * 100))
            grid.set_value(
                "Pct. Intron.", row, format_fp(qc['percentIntronicBases'] * 100)
            )
            grid.set_value(
                "Pct. Intergenic", row, format_fp(qc['percentIntergenicBases'] * 100)
            )
        ...
        if pType['table'] == 'hs':
            ...
            grid.set_value(
                "Pct. Zero Cvg", row, format_fp(qc['zeroCoveragePercent'] * 100)
            )
...
def data_table(pId):
    ...
    r = s.get(
        ...
        auth=(USER, PASSW),
        verify=False,
    )  # , verify=(lims_version == "igo"))
    t = s.get(
        LIMS_API_ROOT + "/LimsRest/getPickListValues?list=Sequencing+QC+Status",
        auth=(USER, PASSW),
        verify=False,
    )  # verify=(lims_version == "igo"))
    ...
    if data[0]['samples'] == []:
        return redirect(
            url_for('page_not_found', pId=data[0]['requestId'], code_error=3)
        )
    ...
    if (
        'RNA' in pType['recipe']
        or 'SMARTerAmpSeq' in pType['recipe']
        or '96Well_SmartSeq2' in pType['recipe']
    ):
        pType['table'] = 'rna'
    ...
    charts_links = {}
    project_indexes = os.listdir(
        APP_STATIC + "/html/PDF"
    )  # os.listdir ( os.path.join("", "html/PDF") )
    for project_index in project_indexes:
        ...
        if image_req == data[0]['requestId']:
            if (
                l[-1].lower() == 'tree.pdf'
                or l[-1].lower() == "heatmap.pdf"
                or l[-1].lower() == "treemap.pdf"
            ):
                charts_links[l[0] + '_' + l[1] + '_' + l[-1].lower()] = (
                    'html/PDF/' + project_index
                )
            elif l[-1].lower() == 'pie.pdf':
                charts_links[l[0] + '_' + l[1] + '_pie.pdf'] = (
                    'html/PDF/' + project_index
                )
...
def isWholeExome(recipe):
    return (
        recipe == "WholeExome-KAPALib"
        or recipe == "WholeExomeSequencing"
        or recipe == "Agilent_v4_51MB_Human"
        or recipe == "WESAnalysis"
        or recipe == "IDT_Exome_v1_FP"
        or recipe == "WES"
    )
...
def get_interops_data():
    ...
    print(runName)
    r = s.get(
        LIMS_API_ROOT + "/LimsRest/getInterOpsData?runId=" + runName,
        auth=(USER, PASSW),
        verify=False,
    )
